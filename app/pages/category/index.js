...
const Category = (props) => {
  ...
  const getData = async () => {
    ...
    set_current_page(1);
    const { collection_id, color, page_type, size, category_id, sub_category_id, price_min, price_max, name,page } = router.query
    
        const res = await fetch(
          `${baseURL}/products?page=1&per_page=${per_page}${name ? `&name=${name}` : ``}${collection_id ? `&collection_id=${collection_id}` : ''}${category_id ? `&category_id=${category_id}` : ``}${sub_category_id ? `&sub_category_id=${sub_category_id}` : ``}${page_type ? `&page_type=${page_type}` : ``}${price_min ? `&price_min=${price_min}` : ``}${price_max ? `&price_max=${price_max}` : ``}${color ? `&color=${color}` : ''}${size ? `&size=${size}` : ``}${currency ? `&currency=${currency.code}` : ''}`
        );
    ...
    <CategoryTags>
      <Container>
        <CategoryPageName>
          ...
          <span>{`(${category.length} Products)`}</span>
        </CategoryPageName>
        {/* {tags.map((tag) => (
                    ...
                    category.map((item, item_index) => (
                      <Grid item md={3} key={item.id}>
                        <CategoryItem >
                          ...
                          {currency && (
                            <p style={{ color: "black", fontSize: "18px", margin: '0px', fontWeight: '500', marginTop: '-20px',marginBottom:'6px' }}>
                              {
